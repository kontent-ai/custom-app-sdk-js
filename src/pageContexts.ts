...
const sharedPageProperties = ["path", "pageTitle"] as const satisfies ReadonlyArray<
  keyof CustomAppPageContextProperties
>;
...
type RawItemEditorPageContext = Pick<
  CustomAppPageContextProperties,
  (typeof itemEditorPageProperties)[number]
>;

type ItemEditorPageContext = Required<
  Pick<
    WithSpecificPage<CustomAppPageContextProperties, "itemEditor">,
    (typeof itemEditorPageProperties)[number]
  >
>;

export const isItemEditorPageContext = (
  context: RawItemEditorPageContext,
): context is ItemEditorPageContext => {
  return (
...
type RawOtherPageContext = Pick<
  CustomAppPageContextProperties,
  (typeof otherPageProperties)[number]
>;

type OtherPageContext = Required<
  Pick<
    WithSpecificPage<CustomAppPageContextProperties, "other">,
    (typeof otherPageProperties)[number]
  >
>;
...
export const isOtherPageContext = (context: RawOtherPageContext): context is OtherPageContext => {
  return (
    ...
    otherPageProperties.every((property) => property in context && context[property] !== undefined)
  );
};
...
type WithSpecificPage<
  T extends CustomAppPageContextProperties,
  Page extends PageContext["currentPage"],
